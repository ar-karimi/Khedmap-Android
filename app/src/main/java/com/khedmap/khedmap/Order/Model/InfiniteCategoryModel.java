...
public class InfiniteCategoryModel  implements InfiniteCategoryContract.ModelCallBack {
    ...
    @Override
    public String getSupportPhoneSharedPref(Context context) {

        //to make sharedPrefManager object
        if (sharedPrefManager == null)
            sharedPrefManager = new SharedPrefManager(context);

        return sharedPrefManager.getSupportPhone();
    }


    @Override
    public void sendGetInfiniteCategoryItemsRequest(Context context, final String requestBody, Response.Listener responseListener, Response.ErrorListener errorListener) {


        RequestQueue queue = Volley.newRequestQueue(context.getApplicationContext());

        String FETCH_CATEGORIES_URL = UtilitiesSingleton.getInstance().getBaseURL() + "fetchCategories";
